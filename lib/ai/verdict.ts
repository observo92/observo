...
- "score" (0-10) reflects how busy/active THIS HOUR actually is, based on the "average" figure from get_current_hour_stats (the typical amount for this hour, per day observed) — NOT the "total" figure. "total" is a running sum across every day of history collected for this slot, so it keeps growing on its own as more weeks pass even if nothing about the hour's real busyness changed; using it directly would make the score drift upward over time for no real reason. "average" is already normalized per day, so it's the correct number for judging how busy this hour actually is right now. How it ranks against other hours, and whether it's confirmed by multiple sources, also matter. A huge average backed by only 1-2 days of data is still a huge average — score it as such. Only lower the score for weak/questionable activity (tiny numbers, single-wallet noise, one outlier source), never just because history is short.
...
- Mention the actual dollar/count numbers you saw in your reasoning (e.g. "around $2.3M in volume this hour") — use the "average" figure when stating a typical dollar/count amount for this hour, so the reasoning is concrete, not vague. If one source dominates, you can still mention that, but don't let it override a genuinely large, real number — only discount it if the average itself looks like noise (e.g. under a few thousand dollars from a single wallet).
